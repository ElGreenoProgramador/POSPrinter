...
```
app/src/main/java/com/example/sunmiprinter/
  SunmiPrinterHelper.kt   – connects to the Sunmi service, coroutine-wraps printBitmap(),
                            printText(), setAlignment(), setFontSize(), bold (raw ESC/POS)
  PrintImageUtils.kt      – scales to a % of printer width, adjusts brightness, dithers to
                            1-bit, and composes a paper-width print preview at a given alignment
  VideoFrameExtractor.kt  – samples video frames at a configurable interval
  MainActivity.kt         – UI: pick photo/video/text, adjustment dialogs, trigger prints
app/src/main/res/layout/
  activity_main.xml       – main screen (3 buttons + preview + status)
  dialog_image_adjust.xml – scale slider + alignment radio group, shown before any image/frame print
  dialog_rich_text.xml    – text input + bold checkbox + font size + alignment
generate_icons.py         – procedurally draws the launcher icon at all densities
```

## Setup

1. Open in Android Studio (Hedgehog+), let Gradle sync — it pulls
   `com.sunmi:printerlibrary:1.0.18` from Sunmi's Maven repo.
2. Build & install on an actual Sunmi V2 (the SDK binds to a system service
   that only exists on Sunmi hardware; it won't find a printer on a generic
   phone/emulator).
3. Grant photo/video permissions when prompted.
4. Tap **Print Photo** or **Print Video** and pick media.

## Notes / things to tune for your use case

- `printerWidthPx` in `MainActivity.kt` defaults to 384 (58mm paper) — change
  to `SunmiPrinterHelper.PRINTER_WIDTH_80MM` (576) if your unit uses 80mm
  paper.
- Dithering is done pixel-by-pixel in Kotlin, which is fine at printer
  resolution (≤576px wide) but would need optimizing (e.g. RenderScript/native)
  if you raise the target width significantly.
- If you need true ESC/POS-level control (custom fonts, QR codes, cash
  drawer, barcode printing, etc.), `SunmiPrinterService` exposes those too —
  see the PDF doc linked above for the full method list.
